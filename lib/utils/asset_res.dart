class AssetRes {
  static String images = "assets/images/";

  static String logoImg = "${images}ebene_logo.png";
  static String twoCircleBgImg = "${images}2_circle_bg_img.png";
  static String halfCircleImg = "${images}half_circle.png";
  static String halfCircleSmallImg = "${images}half_circle_small.png";
  static String doneImg = "${images}done.png";

  ///_________________________ icons _______________________________///
  static String icons = "assets/icons/";

  static String userIcon = "${icons}user.png";
  static String passwordIcon = "${icons}key.png";
  static String closeEyeIcon = "${icons}close_eye.png";
  static String engIcon = "${icons}engImg.png";
  static String emailIcon = "${icons}email.png";
  static String calenderIcon = "${icons}calender.png";
  static String locationIcon = "${icons}location.png";
  static String employeeIcon = "${icons}employee.png";
  static String phoneIcon = "${icons}phone.png";
  static String equalIcon = "${icons}equal.png";
  static String notificationIcon = "${icons}notification.png";
}
