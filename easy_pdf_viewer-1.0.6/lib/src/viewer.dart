...
class _PDFViewerState extends State<PDFViewer> {
  ...
  onZoomChanged(double scale) {
    if (scale == 0) {
      (widget.isOnePage) ? _swipeEnabled = false : _swipeEnabled = true;
    }
    if (scale != 1.0) {
      setState(() {
        _swipeEnabled = false;
  ...
  _loadPage() async {
    if ((widget.isOnePage)
        ? _pages![0] != null
        : _pages![_pageNumber - 1] != null) return;
    setState(() {
      _isLoading = true;
    });
    final data = await widget.document.get(
    ...
    (widget.isOnePage) ? _pages![0] = data : _pages![_pageNumber - 1] = data;
    if (mounted) {
      setState(() {
        _isLoading = false;
      });
          ...
          : Stack(
              children: <Widget>[
                PageView.builder(
                  ...
                  pageSnapping: (widget.isOnePage) ? false : true,
                  allowImplicitScrolling: (widget.isOnePage) ? false : true,
